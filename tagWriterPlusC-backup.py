...
try:
    ...
    config = {
        ...
        "apiKey": "AIzaSyAxtkpUVUuusJNfB7iFKRokjFUmDXHzixY",
        "authDomain": "ruebtags.firebaseapp.com",
        "databaseURL": "https://ruebtags-default-rtdb.firebaseio.com",
        "projectId": "ruebtags",
        "storageBucket": "ruebtags.appspot.com",
        "messagingSenderId": "289046949550",
        "appId": "1:289046949550:web:8c17bef972481efa78873f"
    }
    ...
    # Example usage:
        if not is_internet_available():
            ...
            time.sleep(3)  # Adjust the sleep duration based on your requirements
            continue  # Skip the rest of the loop and restart
        
        init=db.child("tagWriter").child("DID002").child("init").get().val()
        print("Waiting for new ticket...("+str(init)+")")
        if(init=="1"):
            bagNos=db.child("tagWriter").child("DID002").child("bagNos").get().val()
        ...
        #                 print("GUID: " + str(guid))
                    ...
                    data=db.child("tagWriter").child("DID002").child("data").child(tagNo).get().val()
                    print(data)
                    get_data_generate_barcode(data)
                    make_image_and_write()
            init=db.child("tagWriter").child("DID002").child("init").set("0")
        time.sleep(0.5)
